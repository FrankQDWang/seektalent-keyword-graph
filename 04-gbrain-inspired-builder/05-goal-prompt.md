...
- 04-gbrain-inspired-builder/07-quality-gate-schema.md
- 04-gbrain-inspired-builder/08-execution-contract.md
- 04-gbrain-inspired-builder/09-supersession-map.md
...
核心要求：
- 启动前必须执行 `08-execution-contract.md` 里的 preflight commands。路径缺失、权限不对、或 primary mainland JD JSONL 行数不是 9530 时，必须报告 blocker，不能切到 fixture 或合成样本继续。
...
- 建立本项目产品内的 thin harness：CLI/命令执行器只负责确定性执行，包括 import-jds、build-gazetteer、extract-mentions、infer-relations、llm-propose-candidates、review-candidates、probe-provider、build-snapshot、validate-snapshot。
- 建立本项目产品内的 rich skills/docs/configs：schema authoring、corpus import、surface taxonomy、relation rule authoring、LLM candidate review、provider recall probe、anti-hack review、snapshot release。这些是 keyword graph builder 的运行资产，不是 Codex 开发技能。
...
- 必须新增 release candidate surfaces artifact，定义每个进入 CTS final probe 的 surface；不能人为缩小集合来通过 CTS 覆盖。
- 必须新增 graph quality eval gate，并按 `07-quality-gate-schema.md` 输出机器可判定 JSON artifact，覆盖 extraction coverage、generic rejects、ambiguous/risky surfaces、merge samples、bad merge warnings、relation confidence distribution、provider observation coverage、query recall replay cases。
...
- subagent review 必须写入 `08-execution-contract.md` 规定的 `docs/reviews/*-<builder_run_id>.md` artifacts。
- LLM 只能作为本项目 offline builder 的候选生成/确认辅助，通过 builder-only OpenAI-compatible provider 配置调用；第一目标 provider 是阿里云百炼。必须有 explicit config、dry-run、budget、eval、review gate。LLM candidate 必须有 source window、evidence span、model、prompt version、review/validation status；unreviewed candidate 不得进入 release snapshot。runtime 不得调用 LLM。
- Provider probing 开发阶段默认 fake 或 dry-run。但目标最终完成必须在显式人工 gate 后使用真实 CTS，对 release-candidate keyword surfaces 逐个检索并写入 total/status/recall_bucket/observed_at/provider/query_mode/provenance。human gate 必须使用 `08-execution-contract.md` 规定的 gate artifact。最终完成证据绝对不能使用 mock/fake/dry-run/fixture CTS observation。
- readiness report 必须包含 `GBrain reference adoption` 小节，逐个说明读了哪些 GBrain 文件、吸收了哪些原则、落到了哪些实现/测试/报告。
- `docs/superpowers/` 已删除且不得恢复；旧上下文承接关系以 `09-supersession-map.md` 为准。
...
- 第一 slice 必须先落 anti-hack guards/tests，覆盖 runtime import boundary、fixture-specific branch、fixed constant output、broad exception swallowing、hardcoded primary extraction terms、fake readiness、mock CTS final completion evidence。
